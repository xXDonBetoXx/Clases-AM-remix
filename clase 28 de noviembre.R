...
# Agrupamiento jerárquico (jerarquical clustering)
# Dendograma - Gráfica que mandaron
# ¿Cómo interpretar un dendograma?
# cada hoja = es una observacón 
# Interpretación:
# Para dos observaciones cualesquiera , podemos buscar el punto en el árbol donde las ramas que contienen esas dos observaciones se fusionan por primera vez
# mientras las hojas estén más abajo, mayor será la "ligadura" (vínculo) entre ellas
# La altura de la funsión, medida en el eje vertical, indica cuán diferentes son as observaciones
# singleton - conjunto de una sola observación
# ¿Cómo identificar conglomerados (clusters) sobre la base de un dendograma?
# Hacemos un corte horizaontal a través del dendograma 
# Algoritmo de agrupamiento jerárquico
# 1 - Se define una medida de disimilitud entre cada par de observaciones 
# 2- Procede interativamente. Comenzando por la parte inferior del dendograma, cada una de las n observaciones se trta como su propio grupo. Los ods grupos que son más similares entre sí se fusionan para que ahora haya n-1 grupos
# distancia entre observción (o cluster) y cluster - vínculo (linkage); individual, compleyo y promedio

#HC (la gráfica que mandaron)
# trazar el dendograma de agrupamiento jerárquico utilizando el agrupamiento de vínculos completo, único y promedio, con la distancia euclidiana como medida de disimilitud
# dist()
...
plot(hc.single,main = "Enlace único",xlab = "",sub = "", cex=0.9)
# Determinamos las etiquetas de conglomerados para cada observación aociada con un corte del dendograma
cutree(hc.complete, 2)
cutree(hc.average, 2)
cutree(hc.single, 4)
#para escalar las variables antes de realizar el agrupamiento jerárquico 
xsc=scale(x)#parte para estandarizar las variables
plot(hclust(dist(xsc), method = "complete"), main="Agrupamiento jerárquico con características escaladas")
...
library(ISLR2)
ISLR2::NCI60
# datos de micro-arreglos de la límea de células cancerosas NCI60
nci.labs=NCI60$labs # línea celular está etiquetada con un tipo de cáncer
nci.data=NCI60$data
dim(nci.data)
...
nci.complete=hclust(data.dist, method = "complete")
nci.average=hclust(data.dist, method = "average")
nci.single=hclust(data.dist, method = "single")
par(mfrow=c(1,3))
plot(nci.complete,main = "Enlace completo",xlab = "",sub = "", labels = nci.labs)
plot(nci.average,main = "Enlace promedio",xlab = "",sub = "", labels = nci.labs)
plot(nci.single,main = "Enlace único",xlab = "",sub = "", labels = nci.labs)
# las líneas celulares dentro de un sólo tipo de cáncer tienden a agruparse, aunque la agrupación no es perfecta
nci.clusters=cutree(nci.complete, 4)
table(nci.clusters, nci.labs)
